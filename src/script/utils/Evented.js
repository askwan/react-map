...
// import {
//   defineFunction,extend,endsWith
// } from "./util";
function defineFunction(prototype, funs) {
  for (var fn in funs) {
...
Evented._listeners = [];
Evented._oneTimeListeners = [];
Evented._eventedParent = new Evented();
Evented._eventedParentData = {};

defineFunction(Evented.prototype, {
  /**
  ...
  fire(type, data) {
    if (this.listens(type)) {
      ...
      if (this._eventedParent) {
        this._eventedParent.fire(type, extend({}, data, typeof this._eventedParentData === 'function' ? this._eventedParentData() : this._eventedParentData));
      }
